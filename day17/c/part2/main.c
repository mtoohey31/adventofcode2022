...
int main() {
  ...
  int curr_rock, patoff, height, match_count;
  curr_rock = patoff = height = match_count = 0;
  int gridh = 128;

  bool (*grid)[CHAMBERW] = malloc(sizeof(bool[CHAMBERW]) * gridh);
  memset(grid, false, sizeof(bool[CHAMBERW]) * gridh);

  long long shortcut_height = 0;
  // should be laid out as an NROCKS length array of patlen length arrays. each
  // long long[2] contains first the height at the previous instance with this
  // rock and patoff value, then second the iteration i
  long long (*state_record)[2] = malloc(sizeof(long long[2]) * (patlen * NROCKS));
  memset(state_record, 0, sizeof(long long[2]) * (patlen * NROCKS));
  ...
  for (long long i = 0; i < IMAX; i++) {
    ...
    if (shortcut_height == 0) {
      if (state_record[(curr_rock*patlen) + patoff][0] != 0) {
        match_count++;
        if (match_count >= patlen * NROCKS) {
          // we're in a loop!
          long long prev_height = state_record[(curr_rock*patlen) + patoff][0];
          long long prev_i = state_record[(curr_rock*patlen) + patoff][1];
          long long skipped_loops = (IMAX - i) / (i - prev_i);
          shortcut_height = skipped_loops * (((long long) height) - prev_height);
          i += skipped_loops * (i - prev_i);
        }
      } else {
        match_count = 0;
      }

      state_record[(curr_rock*patlen) + patoff][0] = height;
      state_record[(curr_rock*patlen) + patoff][1] = i;
    }
  }

  printf("%lld", ((long long) height) + shortcut_height);
